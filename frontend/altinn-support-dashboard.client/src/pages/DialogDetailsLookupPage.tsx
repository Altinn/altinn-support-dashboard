...
export const DialogDetailsLookupPage = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useDialogDetails(submittedId, environment);

  const revision =
    typeof response?.revision === "string" ? response.revision : undefined;
  ...
  const HIGHLIGHTED_FIELDS: { label: string; value: unknown }[] = response
    ? [
        ...
        {
          label: "Seen since last content update",
          value: response.seenSinceLastContentUpdate,
        },
        { label: "Service owner context", value: response.serviceOwnerContext },
        {
          label: "Service owner labels",
          value: (
            response.serviceOwnerContext as { serviceOwnerLabels?: unknown }
          )?.serviceOwnerLabels,
        },
