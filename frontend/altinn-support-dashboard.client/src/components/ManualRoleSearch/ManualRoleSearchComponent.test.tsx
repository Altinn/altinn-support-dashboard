...
describe('ManualRoleSearchComponent', () => {
    ...
    it('shows "Ingen roller funnet" when search is done but no roles are returned', async () => {
        ...
        const user = userEvent.setup()
        render(<ManualRoleSearchComponent baseUrl="http://mock-api" />)
        await user.type(screen.getByLabelText(/rollehaver/i), '11111111111')
        await user.type(screen.getByLabelText(/rollegiver/i), '222222222')
    ...
    it('displays a table when roles are returned', async () => {
        ...
        const user = userEvent.setup()
        render(<ManualRoleSearchComponent baseUrl="http://mock-api" />)
        await user.type(screen.getByLabelText(/rollehaver/i), '12345678901')
        await user.type(screen.getByLabelText(/rollegiver/i), '987654321')
        await user.click(screen.getByRole('button', { name: /søk/i }))
        await waitFor(() => {
