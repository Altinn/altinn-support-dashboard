...
export const useOrganizationCreation = (environment: string) => {
  ...
  useEffect(() => {
    ...
    if (storedEnvironment) {
      console.log(
        `Bruker lagret miljø fra session storage: ${storedEnvironment}`
      );
  ...
  const [brregResults, setBrregResults] = useState<BrregSearchResponse | null>(
    null
  );
  const [isFetchingOrgDetails, setIsFetchingOrgDetails] =
    useState<boolean>(false);
  const [orgDetails, setOrgDetails] = useState<BrregEnhetsdetaljer | null>(
    null
  );
  ...
  const checkNameExists = useCallback(
    async (shortName: string): Promise<boolean> => {
      ...
      if (
        !shortName ||
        shortName.length < 2 ||
        shortName === lastCheckedName.current
      ) {
        return false;
      }
      ...
      try {
        ...
        const response = await fetch(
          `${getCorrectBaseUrl()}/api/gitea/${activeEnvironment}/organizations/${shortName}/exists`,
          {
            headers: {
        ...
        try {
          ...
          if (exists) {
            setShortNameError(
              `Organisasjonen '${shortName}' eksisterer allerede i Altinn Studio.`
            );
          } else {
            setShortNameError(null);
          }
   ...
   */
  const searchBrreg = useCallback(
    async (searchTerm: string): Promise<BrregSearchResponse | null> => {
   ...
   */
  const validateOrgNumber = useCallback(
    async (orgNumber: string): Promise<boolean> => {
   ...
   */
  const getEnhetsdetaljer = useCallback(
    async (orgNumber: string): Promise<BrregEnhetsdetaljer | null> => {
      if (!orgNumber || orgNumber.length !== 9) {
        return null;
      }
      ...
      if (failedOrgNumbers.current[orgNumber]) {
        console.log(
          `Hopper over API-kall - orgnr ${orgNumber} har feilet tidligere`
        );
        return null;
      }

      if (
        hasSuccessfullyFetchedDetails.current &&
  ...
  ); // Bare avhengig av isFetchingOrgDetails for å unngå doble API-kall
