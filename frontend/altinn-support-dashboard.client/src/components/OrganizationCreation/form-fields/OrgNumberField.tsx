...
export const OrgNumberField: React.FC<OrgNumberFieldProps> = ({
  ...
  const { getEnhetsdetaljer, resetOrgFetchState } =
    useOrganizationCreation('Production'); // Alltid bruk Production for Brreg-søk
  ...
  const fetchOrgDetails = useCallback(
    async (orgNumber: string) => {
      ...
      try {
        ...
        if (!orgDetails) {
          setErrorMessage(
            'Fant ikke organisasjon med dette organisasjonsnummeret, men du kan fortsatt opprette organisasjon'
          );
          setSuccess(false);
          return;
        }
        ...
        setErrorMessage(
          'Det oppstod en teknisk feil ved søk etter organisasjonen. Du kan fortsatt legge inn informasjonen manuelt.'
        );
        setSuccess(false);
      } finally {
        setIsLoading(false);
    ...
    [getEnhetsdetaljer, onFetchOrgDetails, success]
  ...
  const handleReset = () => {
    ...
    console.log(
      'Organizasjonsnummer felt nullstilt - all sporing og caching nullstilt'
    );
  ...
  return (
    <div>
      ...
      >
        ...
        <Tooltip
          content="Organisasjonens organisasjonsnummer i Brønnøysundregistrene (9 siffer). Informasjon hentes automatisk når du skriver inn et gyldig organisasjonsnummer."
          placement="top"
        >
      ...
      {success && (
        <div style={{ marginTop: '8px' }}>
          <Alert data-color="success">
            ...
            <Paragraph>
              Informasjon fra Brønnøysundregistrene er hentet og fylt ut
              automatisk i skjemaet. Kontroller at informasjonen er korrekt før
              du fortsetter.
            </Paragraph>
          </Alert>
        </div>
