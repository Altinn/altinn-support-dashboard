...
public class PartyApiClient : IPartyApiClient
{
    ...
    public async Task<string> GetParty(string lookupValue, bool isOrg, string environmentName)
    {
        ...
        {
            _logger.LogWarning("Party lookup by {LookupType} failed with status code {StatusCode} in environment {EnvironmentName}",  response.StatusCode, environmentName, responseBody);
