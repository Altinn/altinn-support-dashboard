...
public class AltinnApiClient
{
    ...
    public async Task<string> GetPersonRights(string subject, string reportee)
    {
        try
        {
            var requestUrl = $"authorization/rights?subject={subject}&reportee={reportee}&?ForceEIAuthentication";
            Console.WriteLine($"Requesting URL: {_client.BaseAddress}{requestUrl}");
